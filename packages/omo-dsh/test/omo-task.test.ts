...
function mount(tiers: any, extra: any = {}) {
  ...
  apply(ctx, resolved({ provider: "spawn", toolName: "omo_task", backgroundMode: "continuable", tiers, ...extra }));
...
describe("omo-task apply validation", () => {
...
describe("omo-task tool definition", () => {
  ...
  it("adds a system-prompt section in continuable mode", () => {
    ...
    expect(sections[0].name).toBe("tool:omo_task");
  });
});

describe("omo-task execute", () => {
...
describe("omo-task exports", () => {
  it("exports the cordis row contract", () => {
    expect(name).toBe("omo-task");
