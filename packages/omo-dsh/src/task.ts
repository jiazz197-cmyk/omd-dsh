...
/**
 * @module @subagent-router/omo-dsh/task
 *
 * omo-task: tiered subagent delegation tool (the DSH-native equivalent of
 * OmO task(category=...)).
 *
...
/** Runtime schema for the omo-task row. */
const Config = z.object({
  provider: z.string().default("spawn"),
  toolName: z.string().default("omo_task"),
...
function configError(config) {
  ...
  if (tierNames.length === 0) return "omo-task: `tiers` must define at least one tier";
  for (const tierName of tierNames) {
    if (!TIER_NAME_PATTERN.test(tierName)) {
      return "omo-task: invalid tier name \"" + tierName + "\" (must match [a-z0-9][a-z0-9_-]*): " + tierNames.join(", ");
    }
    const filter = config.tiers[tierName].toolFilter;
    if (filter !== undefined && filter.allow === undefined && filter.deny === undefined) {
      return "omo-task: tier \"" + tierName + "\" configures toolFilter but names neither allow nor deny";
    }
  }
  if (config.defaultTier !== undefined && config.tiers[config.defaultTier] === undefined) {
    return "omo-task: defaultTier \"" + config.defaultTier + "\" is not one of the tiers: " + tierNames.join(", ");
  }
  return undefined;
}
...
function resolveTier(config, requested) {
  ...
  if (requested !== undefined) {
    if (config.tiers[requested] === undefined) {
      throw new Error("omo_task: unknown tier \"" + requested + "\" -- valid tiers: " + tierNames.join(", "));
    }
    return requested;
  }
  if (config.defaultTier !== undefined) return config.defaultTier;
  if (tierNames.length === 1) return tierNames[0];
  throw new Error("omo_task: choose a tier for this task -- valid tiers: " + tierNames.join(", "));
}

function apply(ctx, config) {
  if (scopeOf(ctx) === undefined) {
    throw new Error("omo-task: refusing to mount outside a scoped context; mount this row inside an agent preset");
  }
  ...
  const mount = (provider) => {
    if (typeof config.maxDepth === "number" && !provider.capabilities.depthLimit) {
      throw new Error("omo-task: provider \"" + provider.name + "\" cannot enforce maxDepth (no depthLimit capability) -- set maxDepth: provider-managed");
    }
    if (continuable && provider.prepareContinuable === undefined) {
      throw new Error("omo-task: provider \"" + provider.name + "\" does not support backgroundMode: continuable");
    }
    disposeTool = ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        if (!parent) throw new Error("omo_task requires a calling agent (exec.agent was undefined)");
  ...
  else ctx.logger.info("omo-task: subagent provider \"" + config.provider + "\" not registered yet; the \"" + toolName + "\" tool will register when it appears");
